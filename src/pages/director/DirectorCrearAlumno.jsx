...
export default function DirectorCrearAlumno() {
  ...
  const nextStep = () => {
    setFormError(null);
    if (currentStep === 1) {
      if (!form.rut_alumno || !form.nombre_alumno || !form.id_curso || !regionSeleccionada || !form.comuna || !form.calle_numero) {
        setFormError("Por favor, completa todos los datos obligatorios del alumno, incluyendo la dirección.");
        return;
      }
    } else if (currentStep === 2) {
      if (!form.rut_apoderado || !form.nombre_apoderado || !form.parentesco) {
        setFormError("Por favor, completa los datos obligatorios del apoderado titular.");
        return;
      }
    }
  ...
  const handleOpenConfirm = (e) => {
    e.preventDefault();
    setFormError(null);
    if (form.es_pie && !form.diagnostico) {
      setFormError("Debes especificar el diagnóstico principal para inscribirlo en el programa PIE.");
      return;
    }
            ...
            <div key={step} className="flex items-center flex-1 last:flex-none">
              ...
              <div className="ml-3 hidden sm:block">
                <p className={`text-xs font-bold uppercase tracking-wider ${currentStep === step ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400'}`}>
                  {step === 1 ? 'Alumno' : step === 2 ? 'Apoderado' : 'PIE'}
                </p>
              </div>
              {index < 2 && (
          ...
          <form className="space-y-6">
            ...
            {currentStep === 1 && (
              <div className="animate-[fadeIn_0.3s_ease-out]">
                <h3 className="text-lg font-bold text-gray-800 dark:text-white mb-6">Antecedentes del Estudiante</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">RUT Alumno <span className="text-red-500">*</span></label>
                    <input type="text" placeholder="Ej: 21.234.567-8" value={form.rut_alumno} onChange={(e) => { setForm({...form, rut_alumno: formatRUT(e.target.value)}); setFormError(null); }} className="w-full h-11 px-4 rounded-xl border border-gray-300 dark:border-gray-600 bg-transparent text-sm dark:text-white focus:border-indigo-500 outline-none transition-colors" maxLength={12} />
                  </div>
                  <div>
                    ...
                    <input type="text" placeholder="Ej: Anahis Guzmán" value={form.nombre_alumno} onChange={(e) => { setForm({...form, nombre_alumno: e.target.value}); setFormError(null); }} className="w-full h-11 px-4 rounded-xl border border-gray-300 dark:border-gray-600 bg-transparent text-sm dark:text-white focus:border-indigo-500 outline-none transition-colors" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Fecha de Nacimiento</label>
                    <input type="date" value={form.fecha_nacimiento} onChange={(e) => setForm({...form, fecha_nacimiento: e.target.value})} className="w-full h-11 px-4 rounded-xl border border-gray-300 dark:border-gray-600 bg-transparent text-sm dark:text-white focus:border-indigo-500 outline-none transition-colors scheme-light-dark" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Asignar Curso de Destino <span className="text-red-500">*</span></label>
                    <select value={form.id_curso} onChange={(e) => { setForm({...form, id_curso: e.target.value}); setFormError(null); }} className="w-full h-11 px-4 rounded-xl border border-gray-300 dark:border-gray-600 bg-transparent text-sm dark:text-white focus:border-indigo-500 outline-none transition-colors">
                  ...
                  <div className="md:col-span-2 mt-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-xl bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700">
                      <div>
                        <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Región <span className="text-red-500">*</span></label>
                        <select value={regionSeleccionada} onChange={(e) => { handleRegionChange(e); setFormError(null); }} className="w-full h-10 px-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-transparent text-sm dark:text-white focus:border-indigo-500 outline-none transition-colors">
                          <option value="" disabled className="dark:bg-gray-800">Seleccione Región</option>
                          {Object.keys(regionesChile).map(region => <option key={region} value={region} className="dark:bg-gray-800">{region}</option>)}
                        </select>
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Comuna <span className="text-red-500">*</span></label>
                        <select value={form.comuna} onChange={(e) => { setForm({...form, comuna: e.target.value}); setFormError(null); }} disabled={!regionSeleccionada} className="w-full h-10 px-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-transparent text-sm dark:text-white focus:border-indigo-500 outline-none transition-colors disabled:opacity-50">
                          <option value="" disabled className="dark:bg-gray-800">Seleccione Comuna</option>
                          {comunasDisponibles.map(comuna => <option key={comuna} value={comuna} className="dark:bg-gray-800">{comuna}</option>)}
                        </select>
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Calle y Número <span className="text-red-500">*</span></label>
                        <input type="text" placeholder="Ej: Los Leones 1234" value={form.calle_numero} onChange={(e) => { setForm({...form, calle_numero: e.target.value}); setFormError(null); }} className="w-full h-10 px-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-transparent text-sm dark:text-white focus:border-indigo-500 outline-none transition-colors" />
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {currentStep === 2 && (
              <div className="animate-[fadeIn_0.3s_ease-out]">
                <h3 className="text-lg font-bold text-gray-800 dark:text-white mb-6">Antecedentes del Apoderado</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">RUT Apoderado <span className="text-red-500">*</span></label>
                    <input type="text" placeholder="Ej: 15.234.567-8" value={form.rut_apoderado} onChange={(e) => { setForm({...form, rut_apoderado: formatRUT(e.target.value)}); setFormError(null); }} className="w-full h-11 px-4 rounded-xl border border-gray-300 dark:border-gray-600 bg-transparent text-sm dark:text-white focus:border-indigo-500 outline-none transition-colors" maxLength={12} />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Nombre Completo del Apoderado <span className="text-red-500">*</span></label>
                    <input type="text" placeholder="Ej: Eduardo Guzmán Vega" value={form.nombre_apoderado} onChange={(e) => { setForm({...form, nombre_apoderado: e.target.value}); setFormError(null); }} className="w-full h-11 px-4 rounded-xl border border-gray-300 dark:border-gray-600 bg-transparent text-sm dark:text-white focus:border-indigo-500 outline-none transition-colors" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Parentesco Relacional <span className="text-red-500">*</span></label>
                    <select value={form.parentesco} onChange={(e) => { setForm({...form, parentesco: e.target.value}); setFormError(null); }} className="w-full h-11 px-4 rounded-xl border border-gray-300 dark:border-gray-600 bg-transparent text-sm dark:text-white focus:border-indigo-500 outline-none transition-colors">
                  ...
                  <div>
                    ...
                    <input type="email" placeholder="correo@ejemplo.com" value={form.email_apoderado} onChange={(e) => setForm({...form, email_apoderado: e.target.value})} className="w-full h-11 px-4 rounded-xl border border-gray-300 dark:border-gray-600 bg-transparent text-sm dark:text-white focus:border-indigo-500 outline-none transition-colors" />
                  </div>
                  <div>
            ...
            {currentStep === 3 && (
              <div className="animate-[fadeIn_0.3s_ease-out]">
                <h3 className="text-lg font-bold text-gray-800 dark:text-white mb-6">Programa de Integración Escolar (PIE)</h3>
                ...
                <div className="bg-indigo-50/50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-800/30 rounded-xl p-6">
                  <label className="flex items-center gap-3 cursor-pointer mb-2">
                    <input type="checkbox" checked={form.es_pie} onChange={(e) => setForm({...form, es_pie: e.target.checked})} className="w-5 h-5 text-indigo-600 rounded border-gray-300 focus:ring-indigo-500" />
                    <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">
                      Inscribir estudiante en el Programa PIE
                    </span>
                  </label>
                  <p className="text-xs text-gray-500 dark:text-gray-400 ml-8 mb-4">Activar solo si el estudiante requiere evaluación y apoyo especializado.</p>

                  {form.es_pie && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-4 pt-5 border-t border-indigo-200 dark:border-indigo-800/50 animate-[fadeIn_0.3s_ease-out]">
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Tipo de Necesidad</label>
                        <select value={form.tipo_necesidad} onChange={(e) => setForm({...form, tipo_necesidad: e.target.value})} className="w-full h-11 px-4 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm dark:text-white focus:border-indigo-500 outline-none transition-colors">
                          <option value="NEET">NEET (Transitoria)</option>
                          <option value="NEEP">NEEP (Permanente)</option>
                        </select>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Diagnóstico Principal <span className="text-red-500">*</span></label>
                        <input type="text" placeholder="Ej: TDAH, TEA, TEL..." value={form.diagnostico} onChange={(e) => { setForm({...form, diagnostico: e.target.value}); setFormError(null); }} className="w-full h-11 px-4 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm dark:text-white focus:border-indigo-500 outline-none transition-colors" />
                      </div>
                    </div>
                  )}
                </div>
      ...
      {isConfirmModalOpen && (
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-gray-900/50 backdrop-blur-sm px-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-sm p-6 border border-gray-200 dark:border-gray-700 animate-[fadeInUp_0.2s_ease-out] relative overflow-hidden">
            {isSaving && <BackdropLoader mensaje="Matriculando..." />}
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleMatricular} disabled={isSaving} className="flex-1 py-2.5 rounded-lg bg-emerald-600 text-white font-medium text-sm hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-600/20">
                Sí, matricular
              </button>
            </div>
          </div>
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-900/50 rounded-xl p-3 border border-gray-200 dark:border-gray-700 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-800 dark:text-gray-200">Usuario: <span className="font-bold text-blue-600 dark:text-blue-400">{successData.alumno.rut}</span></p>
                  <p className="text-sm font-medium text-gray-800 dark:text-gray-200">Clave: <span className="font-bold text-blue-600 dark:text-blue-400">{successData.alumno.clave}</span></p>
                </div>
              </div>
              ...
              <div className="bg-gray-50 dark:bg-gray-900/50 rounded-xl p-3 border border-gray-200 dark:border-gray-700 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-800 dark:text-gray-200">Usuario: <span className="font-bold text-indigo-600 dark:text-indigo-400">{successData.apoderado.rut}</span></p>
                  <p className="text-sm font-medium text-gray-800 dark:text-gray-200">Clave: <span className="font-bold text-indigo-600 dark:text-indigo-400">{successData.apoderado.clave}</span></p>
                </div>
              </div>
            </div>

            <button
              onClick={() => { setSuccessData(null); navigate('/panel/director/alumnos'); }}
