        ...
        <div className="py-6 flex-1 overflow-y-auto">
          <div className={`mb-4 ${isCollapsed ? 'px-2' : 'px-4'}`}>
            ...
            <nav className="space-y-1.5">
              ...
              {/* GESTIÓN DE TAREAS (Desplegable) */}
              <div className="relative pt-2">
                <button 
                  onClick={() => toggleSubmenu('tareas')}
                  title={isCollapsed ? "Gestión de Tareas" : ""}
                  className={`w-full flex items-center py-2.5 text-sm font-medium rounded-lg transition-colors ${isCollapsed ? 'justify-center px-0' : 'px-4'} ${
                    openSubmenus.tareas || isItemActive('/panel/profesor/tareas')
                    ? 'text-blue-600 bg-blue-50 dark:text-blue-400 dark:bg-blue-900/20' 
                    : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'
                  }`}
                >
                ...
                <div className={`overflow-hidden transition-all duration-300 ${openSubmenus.tareas ? 'max-h-40 mt-1' : 'max-h-0'}`}>
                  <div className={`space-y-1 ${isCollapsed ? 'px-0' : 'pl-11 pr-2'}`}>
                    ...
                    >
                      {isCollapsed ? <span className="font-bold text-xs uppercase">VT</span> : <span>Ver Tareas</span>}
                    </button>
                    <button 
                      onClick={() => { navigate('/panel/profesor/tareas/nueva'); setIsMobileMenuOpen(false); }}
                      title={isCollapsed ? "Crear Tarea" : ""}
                    ...
                    >
                      {isCollapsed ? <span className="font-bold text-xs uppercase">CT</span> : <span>Crear Tarea</span>}
                    </button>
                  </div>
                </div>
              </div>
