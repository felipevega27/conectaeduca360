        ...
        <div className="py-6 flex-1 overflow-y-auto">
          <div className={`mb-4 ${isCollapsed ? 'px-2' : 'px-4'}`}>
            <nav className="space-y-1.5">
              ...
              {/* DOCUMENTOS */}
              <button 
                onClick={() => { navigate('/panel/alumno/documentos'); setIsMobileMenuOpen(false); }}
                title={isCollapsed ? "Trámites y Certificados" : ""}
                className={`w-full flex items-center py-2.5 text-sm font-medium rounded-lg transition-colors ${isCollapsed ? 'justify-center px-0' : 'px-4'} ${
                  isItemActive('/panel/alumno/documentos') 
                  ? 'text-blue-600 bg-blue-50 dark:text-blue-400 dark:bg-blue-900/20' 
                  : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'
                }`}
              >
                <svg className={`h-5 w-5 shrink-0 ${isCollapsed ? '' : 'mr-3'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                {!isCollapsed && <span>Trámites y Certificados</span>}
              </button>
            </nav>
          </div>
